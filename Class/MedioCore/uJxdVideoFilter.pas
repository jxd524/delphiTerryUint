...
type
  ...
  TMyTestFilter = class(TBCTransInPlaceFilter)
  public
    constructor Create(out hr: HRESULT);
    destructor  Destroy; override;
...
function TMyTestFilter.CheckInputType(mtIn: PAMMediaType): HRESULT;
begin
  Result := S_OK;
end;

function TMyTestFilter.Copy2(Source: IMediaSample): IMediaSample;
var
  Start, Stop: TReferenceTime;
  Time: boolean;
  pStartTime, pEndTime: PReferenceTime;
...
constructor TMyTestFilter.Create(out hr: HRESULT);
begin
  inherited Create( ClassName, nil, CtOverLayGUID, hr, True );
  hr := S_OK;
end;

procedure TMyTestFilter.DeliverOutPin2(Sample: IMediaSample);
var
  Sample2: IMediaSample;
begin
  if FOutPin2.IsConnected then
...
destructor TMyTestFilter.Destroy;
begin

  inherited;
end;

function TMyTestFilter.GetPin(n: integer): TBCBasePin;
var
  hr: HRESULT;
begin
  if n in [0, 1] then
...
function TMyTestFilter.GetPinCount: integer;
begin
  Result := 3;
end;

function TMyTestFilter.Transform(Sample: IMediaSample): HRESULT;
begin
//  inherited;
  //DeliverOutPin2();
  FOutput.Deliver( Sample );
  DeliverOutPin2( Sample );
  Result := S_OK;
end;
