...
uses
  Windows, Classes, SysUtils, DirectShow9, BaseClass, Math, Graphics, DSUtil, uJxdVideoFormat
  {$ifdef OverlayDebug}
  , uDebugInfo
  {$endIf};
...
type
  ...
  private
    ...
    procedure SetOnDraw(const Value: TOnRgb24FilterDraw);
  public
    property OnDraw: TOnRgb24FilterDraw read FOnDraw write SetOnDraw;
  end;
...
procedure TYuY2OverlayFilter.SetOnDraw(const Value: TOnRgb24FilterDraw);
begin
  FOnDraw := Value;
end;
...
procedure TRgb24OverlayFilter.SetOnDraw(const Value: TOnRgb24FilterDraw);
begin
  FOnDraw := Value;
end;
