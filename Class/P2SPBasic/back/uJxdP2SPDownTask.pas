...
uses
  Windows, Classes, SysUtils, uJxdHashCalc, uJxdDataStream, uJxdFileSegmentStream, uJxdUdpDefine, uJxdUdpSynchroBasic, uJxdServerManage,
  idHttp, IdComponent, IdURI;
...
type
  ...
  public
    ...
    procedure CheckFileStream; //确定在参数有效的情况下生成文件流, 不管是有任务激活与否
  ...
  private
    ...
    FSegmentTable: TxdFileSegmentTable; //文件分段表
    ...
    function  GetHashCheckSegmentPeer: PP2PSource; //获取提供Hash分段验证的节点
    ...
    function  GetFileInfoByP2P(p: PP2PSource): Boolean; //向指定用户获取文件信息
    ...
    //控制检查
    function  CheckTaskCompleted: Boolean; //检查文件是否已经下载完成
  ...
  published
    ...
    property CheckP2PDownProgressSpaceTime: Cardinal read FCheckP2PDownProgressSpaceTime write FCheckP2PDownProgressSpaceTime;//请求P2P下载进度
...
begin
  ...
  begin
    //关键FSegmentTable后，则 FSegmentTable 由FFileStream在释放时，去释放
...
begin
  ...
  begin
    FSegmentTable := TxdFileSegmentTable.Create( AFileSize, nil, ASegmentSize );
...
function TxdP2SPDownTask.CheckTaskCompleted: Boolean;
var
  hd: TxdHash;
  bFinished: Boolean;
begin
  ...
  begin
    bFinished := True;
    if not HashCompare(CtEmptyHash, FFileHash) then
    begin
      CalcFileHash( FFileStream, hd, nil );
      if not HashCompare(hd, FFileHash) then
        bFinished := False;
    end;
    ...
    begin
      if HashCompare(CtEmptyHash, FFileHash) then
        CalcFileHash( FFileStream, FFileHash, nil );
      FFinished := True;
      Active := False;
...
begin
  ...
  try
    ...
    begin
      ...
      if (p^.FIP = AIP) and (p^.FPort = APort) then
      begin
        //如果查询不到信息，此数据源本身也没有验证成功，则删除此数据源
...
//  if (dwTime > FLastExcuteTime) and (dwTime - FLastExcuteTime < CtMinSpaceTime) then Exit;
  ...
  {$ENDIF}
  if not Assigned(FSegmentTable) then Exit;
  if CheckTaskCompleted then Exit;
...
begin
  if not FActive then
  begin
    OutputDebugString( '任务已停止，直接丢弃数据' );
    Exit;
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        //设置为成功, 如果是服务器源则直接请求数据，否则查询数据源进度
...
function TxdP2SPDownTask.GetFileInfoByP2P(p: PP2PSource): Boolean;
  ...
  //发送 CtCmd_QueryFileInfo 命令，获取文件信息
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          if Result^.FTotalRecvByteCount < p^.FTotalRecvByteCount then
            Result := p;
        end
        else
          Result := p;
