...
uses
  ...
  uJxdUdpBasic, uJxdUdpsynchroBasic, uJxdOnlineUserManage, uJxdServerManage, uJxdUdpDefine;
...
type
  ...
  private
    ...
    {OnlineUserManage 事件}
    procedure DoDeletedUser(const ApUserInfo: POnlineUserInfo);
  private
    FMinUserID: Cardinal;
...
procedure TxdUdpOnlineServer.DoDeletedUser(const ApUserInfo: POnlineUserInfo);
var
  i: Integer;
...
begin
  ...
  begin
    ...
    notifyCmd.FShutDownID := ApUserInfo^.FUserID;
...
begin
  ...
  if Replysign = rsError then Exit; //命令发送者无注册, 不处理

  oSendStream := TxdStaticMemory_512Byte.Create;
  try
    //回复 CtCmdReply_CallMe
    ...
    begin
      ...
      //1：向UserA, UserB客户端发送 CtCmd_CallFriend
      oSendStream.Clear;
      AddCmdHead( oSendStream, CtCmd_CallFriend );
      oSendStream.WriteLong( UserB, CtUserNetInfoSize );
