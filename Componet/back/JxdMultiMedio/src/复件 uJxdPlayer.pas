...
begin
  ...
  begin
    if IsContainVideo and Succeeded(FFilterGraph.QueryInterface(IID_IVideoWindow, pVideo)) then
    begin
      try

        pVideo.put_MessageDrain(Handle);
        pVideo.put_WindowStyle(WS_CHILD or WS_CLIPSIBLINGS);
        pVideo.put_WindowStyleEx(WS_EX_TOPMOST);
        pVideo.get_FullScreenMode( bFull );
        pVideo.put_FullScreenMode( not bFull );
      finally
        pVideo := nil;
      end;
    end;
