...
interface
  uses
    Windows, Messages, SysUtils, Classes, CommCtrl, TLHelp32, Psapi, ShlObj, WinInet, Forms, ShlwAPI;
  ...
  function Regedit(AFileName: string): Cardinal;
  ...
  function GetAbsolutePathEx(BasePath, RelativePath:string): string; //取绝对路径的函数
  function GetRelativePath(const Path, AFileName: string): string; //绝对路径转换为相对路径的函数
...
begin
  while True do
  begin
    if AMilliSecond - 10 > 0 then
    begin
      Sleep(10);
      AMilliSecond := AMilliSecond - 10;
    end
    else
    begin
      Sleep(AMilliSecond);
...
begin
  ...
  while True do
  begin
    strName := Format('%s%d%s.%s', ['xd', GetRandomNum(0, 999999), '~XPP', AExtentName]);
...
function Regedit(AFileName: string): Cardinal;
const
  CtRegeditCMD = 'regedit.exe /s "%s"';
begin
  Result := WinExec(PChar(format(CtRegeditCMD,[AFileName])), SW_HIDE);
end;
...
function GetRelativePath(const Path, AFileName: string): string; 
...
begin
   PathRelativePathTo(p, PChar(Path), GetAttr(False), PChar(AFileName), GetAttr(True));
   Result := StrPas(p);
end;
