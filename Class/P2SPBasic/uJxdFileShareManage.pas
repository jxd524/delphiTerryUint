...
说    明:
开始时间: 2011-04-08
修改时间: 2011-09-12 (最后修改时间)
类说明  :
    管理本地文件的共享，记录文件的HASH，验证文件用到的HASH，
      验证文件HASH的分段大小为 SegmentSize * 32 默认情况，HASH检验分段大小为: 8.0M (256K * 32)
      大部分情况下是不需要用到分段HASH检验的。提供分段HASH检验功能

      Hash计算使用函数： HashFun_BKDR

    内存使用动态申请的，如想优化，可做不定长内存分配处理
...
const
  CtCalcHashSegmentCount = 32; // * SegmentSize 为一个大分段的HASH验证大小使用的分段数量
...
type
  ...
  TOnFileShareInfo = procedure(Sender: TObject; const Ap: PFileShareInfo) of object;
  {$M+}
  ...
  public
    ...
    //添加一个本地的文件来共享, 不对文件进行判断
    function  AddLocalFileToShare(const AFileName: string; const AShareTag: string = ''; const ASegmentSize: Integer = 0): Boolean; overload;
    function  AddLocalFileToShare(const AStream: TxdFileSegmentStream; const AShareTag: string): Boolean; overload;

    procedure LockShareManage; inline;
    procedure UnLockShareManage; inline;

    {查找共享信息，非线程安全}
    procedure LoopShareFileInfo(ALoop: TOnLoopNode); //遍历共享表中所有内容
    function  FindShareInfoByFileHash(const AHash: TxdHash; const ADel: Boolean = False): PFileShareInfo;
    function  FindShareInfoByWebHash(const AHash: TxdHash; const ADel: Boolean = False): PFileShareInfo;
  private
    FCurFindWebHash: TxdHash;
    FCurFindWebHashDel: Boolean;
  ...
  protected
    ...
    procedure DoNotifyAddShareSuccess(const Ap: PFileShareInfo); //通知添加到列表成功
    ...
    procedure CalcHashCheckTable(stream: TxdP2SPFileStreamBasic; const Ap: PFileShareInfo);
  ...
  private
    ...
    FOnFileShareInfo: TOnFileShareInfo;
  ...
  published
    ...
    property OnFileShareInfo: TOnFileShareInfo read FOnFileShareInfo write FOnFileShareInfo;
  end;
  {$M-}
...
function TxdFileShareManage.AddLocalFileToShare(const AStream: TxdFileSegmentStream; const AShareTag: string): Boolean;
var
  p: PFileShareInfo;
  nCheckHashSegmentCount: Integer;
  nSegSize: Integer;
  ...
  nFileSize: Int64;
begin
  Result := Assigned(AStream) and Assigned(AStream.SegmentTable);
  if not Result then Exit;

  nFileSize := AStream.SegmentTable.FileSize;
  if nFileSize <= MinShareFileSize then 
  begin
    Result := False;
    Exit;
  end;
  
  if AStream.SegmentTable.SegmentSize <= 0 then
    nSegSize := CtSegmentDefaultSize
  else
    nSegSize := AStream.SegmentTable.SegmentSize;
  nCalcHashSegmentSize := nSegSize * CtCalcHashSegmentCount;
  if nCalcHashSegmentSize > nFileSize then
    nCalcHashSegmentSize := nFileSize;
  nCheckHashSegmentCount := (nFileSize + nCalcHashSegmentSize - 1) div nCalcHashSegmentSize;
  p := GetFileShareMem( nCheckHashSegmentCount );
  p^.FShareTag := AShareTag;
  p^.FFileName := AStream.RenameFileName;
  if p^.FFileName = '' then
    p^.FFileName := AStream.FileName;
  p^.FFileSize := nFileSize;
  p^.FSegmentSize := nSegSize;
  p^.FHashCheckSegmentSize := nCalcHashSegmentSize;
  p^.FHashCheckSegmentCount := nCheckHashSegmentCount;
  p^.FFileHash := AStream.FileHash;
  p^.FWebHash := AStream.WebHash;
...
procedure TxdFileShareManage.CalcHashCheckTable(stream: TxdP2SPFileStreamBasic; const Ap: PFileShareInfo);
var
  i: Integer;
  nSize: Integer;
  buf: PByte;
begin
  GetMem( buf, Ap^.FHashCheckSegmentSize );
  try
    for i := 0 to Ap^.FHashCheckSegmentCount - 1 do
    begin
      if HashCompare(CtEmptyHash, Ap^.FHashCheckTable[i]) then
      begin
        if i = Ap^.FHashCheckSegmentCount - 1 then
          nSize := Ap^.FFileSize - i * Ap^.FHashCheckSegmentSize
        else
          nSize := Ap^.FHashCheckSegmentSize;
        Stream.ReadBuffer( i * Ap^.FHashCheckSegmentSize, nSize, buf  );
        Ap^.FHashCheckTable[i] := HashBuffer( buf, nSize );
      end;
      if not Active then Break;
    end;
  finally
    FreeMem( buf, Ap^.FHashCheckSegmentSize );
  end;
end;
...
procedure TxdFileShareManage.DoNotifyAddShareSuccess(const Ap: PFileShareInfo);
begin
  OutputDebugString( '成功添加一个共享信息' ); 
  if Assigned(FOnFileShareInfo) then
    FOnFileShareInfo(Self, Ap);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      try
        if Active and IsEmptyHash(P^.FFileHash) then
          CalcFileHash( Stream, p^.FFileHash, @FCloseingManage );
        if Active and IsEmptyHash(p^.FWebHash) then
...
function TxdFileShareManage.FindShareInfoByFileHash(const AHash: TxdHash; const ADel: Boolean): PFileShareInfo;
var
  pNode: PHashNode;
  nID: Cardinal;
  p: PFileShareInfo;
  ...
  try
    ...
    begin
      ...
      begin
        Result := p;
        if ADel then
          FShareFileList.FindDelete( pNode );
        Break;
      end;
      pNode := FShareFileList.FindNext( pNode );
    end;
...
function TxdFileShareManage.FindShareInfoByWebHash(const AHash: TxdHash; const ADel: Boolean): PFileShareInfo;
begin
//  EnterCriticalSection( FShareLock );
  try
    FCurFindWebHash := AHash;
    FCurFindWebHashDel := ADel;
    FFindShareInfo := nil;
...
procedure TxdFileShareManage.LockShareManage;
begin
  EnterCriticalSection( FShareLock );
end;
...
procedure TxdFileShareManage.UnLockShareManage;
begin
  LeaveCriticalSection( FShareLock );
end;
