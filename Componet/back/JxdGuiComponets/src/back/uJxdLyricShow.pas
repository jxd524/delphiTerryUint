...
uses
  SysUtils, Windows, Classes, Controls, uJxdParseLyric, Graphics, Messages;
...
type
  ...
  TxdLyricShow = class(TGraphicControl)
  private
...
procedure TxdLyricShow.CMFontChanged(var Message: TMessage);
begin
  Canvas.Font.Assign(Font);
  FBitmap.Canvas.Font.Assign(Font);
end;

constructor TxdLyricShow.Create(AOwner: TComponent);
begin
  inherited;
  FMouseMove := False;
...
procedure TxdLyricShow.DrawCursor;
var
  XOffset: Integer;
  R: TRect;
  TextStr: string;
...
procedure TxdLyricShow.FileColorTable;
var
  I, Step: Integer;
...
function TxdLyricShow.GetLyricMaxLength: Integer;
var
  i: Integer;
  LineString: string;
  nTmpLength: Integer;
...
function TxdLyricShow.HeightToTime(AHeight: Integer): Integer;
begin
  Result := (AHeight * FCurLineInfo.ScrollTime) div Canvas.TextHeight(FCurLineInfo.ScrollString + ' ');
end;

procedure TxdLyricShow.InitBitmap;
begin
  FBitmap.Width := Width;
...
procedure TxdLyricShow.LMouseDragingLRC(X, Y: Integer);
begin
...
procedure TxdLyricShow.LoadLyric(const ALyricName: string);
begin
  FParseLyric.LoadLyricFile(ALyricName);
end;

procedure TxdLyricShow.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
...
procedure TxdLyricShow.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
...
procedure TxdLyricShow.MouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
...
procedure TxdLyricShow.ScrollLyricShow(ATime: Integer);
begin
...
procedure TxdLyricShow.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
...
procedure TxdLyricShow.SetBrightColor(const Value: TColor);
begin
...
procedure TxdLyricShow.SetLyricShowKind(const Value: TLyricShowKind);
begin
  if Value <> FKind then
  begin
    FKind := Value;
...
procedure TxdLyricShow.SetNormFontColor(const Value: TColor);
begin
...
procedure TxdLyricShow.SrcollHorizontal(ATime: Integer; ADrawCursor: Boolean);
var
  CurWidth: Integer;
  LineInfo: TLineInfo;
  CurOffset: Integer;
...
procedure TxdLyricShow.SrcollSingLine(ATime: Integer);
var
  CurWidth: Integer;
begin
  if not FParseLyric.GetLineInfo(ATime, FCurLineInfo) then
...
procedure TxdLyricShow.SrcollVertical(ATime: Integer; ADrawCursor: Boolean);
var
  CurHeight: Integer;
  LineInfo: TLineInfo;
  CurOffset: Integer;
...
function TxdLyricShow.TextHeight(const AText: string): Integer;
var
  Size: TSize;
begin
...
procedure TxdLyricShow.TextOutLyric(AText: string; AColor: TColor;
...
function TxdLyricShow.WidthToTime(AWidth: Integer): Integer;
begin
