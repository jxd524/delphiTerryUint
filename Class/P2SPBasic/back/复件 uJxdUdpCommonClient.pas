...
uses
  ...
  uJxdFileShareManage, uJxdP2SPDownTaskManage, uJxdServerManage;
...
type
  ...
  TP2PConWaitEventInfo = record
    FUserID: Cardinal;
    FParamData: Pointer; //参数指针数据
    FOnP2PConnected: TOnP2PConnected;
    FActiveTime: Cardinal; //无响应十秒之后自动释放
  end;
  ...
  public
    ...
    function  ConnectToClient(const AUserID: Cardinal; const ANotify: TOnP2PConnected = nil; const AParam: Pointer = nil): TConnectState; //请求P2P连接
  ...
  protected
    ...
    procedure DoHandleRecvBuffer(const AIP: Cardinal; const APort: Word; const ABuffer: pAnsiChar; const ABufLen: Cardinal;
      const AIsSynchroCmd: Boolean; const ASynchroID: Word); override;
  private
    FP2PWaitEventList: TThreadList; //启动服务时创建，关闭服务时释放
  ...
  private
    ...
    FDownTaskManage: TxdP2SPDownTaskManage;
    FLogin: Boolean;
    FSafeSign: Cardinal;
    FServerIP: Cardinal;
    FServerPort: Word;
  ...
  published
    ...
    property DownTaskManage: TxdP2SPDownTaskManage read FDownTaskManage write FDownTaskManage;
...
begin
  inherited;
  FServerManage := nil;
  FFileShareManage := nil;
  FUploadFileManage := nil;
  FDownTaskManage := nil;
...
begin
  ...
  if not Assigned(FServerManage) or not Assigned(FFileShareManage) or
     not Assigned(FUploadFileManage) or not Assigned(FDownTaskManage)  then
  begin
    raise EUDPError.Create( '必须先设置相关对象: TxdUdpCommonClient.DoBeforOpenUDP' );
  end;
...
begin
  ...
  begin
    DoErrorInfo( '接收到无效的P2PDisconnected命令' );
    Exit;
  end;
