...
uses
  Windows, SysUtils, uJxdCmdDefine, uJxdHashCalc, uJxdDataStruct, uJxdDataStream, uJxdUdpSynchroBasic, uJxdMemoryManage;
...
type
  ...
  TUserListInfo = record
    FUserInfo: PUserTableInfo;
    FNext: PUserListInfo;
  end;

  THashListInfo = record
...
begin
  ...
  try
    ...
    while Stream.Position >= CtHashSize * 2 do
    begin
      ...
      begin
        if HashCompare(pHash^.FFileHash, CtEmptyHash) and not HashCompare(FileHash, CtEmptyHash) then
        begin
          pHash^.FFileHash := FileHash;
          FFileHashList.Add( HashToID(pHash^.FFileHash), pHash );
        end;
        if HashCompare(pHash^.FWebHash, CtEmptyHash) and not HashCompare(WebHash, CtEmptyHash) then
        begin
          pHash^.FWebHash := WebHash;
          FWebHashList.Add( HashToID(pHash^.FWebHash), pHash );
        end;
