...
function CalcCRC32(ABuffer: PChar; ALen: Integer): DWORD; overload;
...
//计算CRC16
function CalcCRC16(ABuffer: PChar; ALen: Integer): WORD;
//加密函数  返回加密前数据的CRC32
function EncodeBuffer(ABuffer: PChar; ALen: Integer): DWORD;
//解密函数  ACRC32是数据的CRC32,成功解密,数据并未破坏则返回True,否则返回False
function DecodeBuffer(ACRC32: DWORD; ABuffer: PChar; ALen: Integer): Boolean;
...
function EncryptStr(const S: string; const Key: string = '!@#$%^abcde'): string;
function DecryptStr(const S: string; const Key: string = '!@#$%^abcde'): string;
function EncryptFile(const AFileName: string; const Key: string = '!@#$%^abcde'):Boolean;
function DecryptFile(const AFileName: string; const Key: string = '!@#$%^abcde'): TStream;
function DecryptFileEx(const AInFileName,AOutFileName: string; const Key: string = '!@#$%^abcde'): Boolean;
...
procedure xdEncryptBuffer(ApBuf: PAnsiChar; const ALen: Integer; const AKey: string = 'skd@#@#0923K'); overload;
//procedure xdEncryptBuffer(ApBuf: PAnsiChar; const ALen: Integer; const AKey: array of Char); overload;
procedure xdDecryptBuffer(ApBuf: PAnsiChar; const ALen: Integer; const AKey: string = 'skd@#@#0923K');
...
//const
//  CLen = 8;
//  CKey: array[0..CLen - 1] of Byte = (56, 12, 77, 34, 67, 10, 56, 188);
var
  I: Integer;
  P: PByte;
begin
...
begin
  ...
  Result := S;
  for I := 1 to Length(S) do
    Result[I] := Chr((Ord(S[I]) + Ord(Key[I mod KeyLen + 1])) mod 256);
end;

procedure xdEncryptBuffer(ApBuf: PAnsiChar; const ALen: Integer; const AKey: string = 'skd@#@#0923K');
var
  i, nKeyLen: Integer;
begin
  nKeyLen := Length( AKey );
  if nKeyLen = 0 then Exit;
  for i := 0 to ALen - 1 do
    ApBuf[i] := Chr( (Ord(ApBuf[i]) + Ord(AKey[i mod nKeyLen + 1])) mod 256 );
end;

//procedure xdEncryptBuffer(ApBuf: PAnsiChar; const ALen: Integer; const AKey: array of Char);
//var
//  i, nKeyLen: Integer;
//begin
//  nKeyLen := Length( AKey );
...
begin
  ...
  Result := S;
  for I := 1 to Length(S) do
    Result[I] := Chr((Ord(S[I]) + 256 - Ord(Key[I mod KeyLen + 1])) mod 256);
end;
...
begin
  ...
  if nKeyLen = 0 then Exit;
  for i := 0 to ALen - 1 do
    ApBuf[i] := Chr( (Ord(ApBuf[i]) + 256 - Ord(AKey[i mod nKeyLen + 1])) mod 256 );
end;

function EncryptFile(const AFileName, Key: string):Boolean;
var
  S: string;
begin
  Result := False;
...
function DecryptFile(const AFileName, Key: string): TStream;
var
  S: string;
begin
  Result := nil;
...
begin
  Result := False;
  try
    MStream := DecryptFile(AInFileName);
    if MStream <> nil then
    begin
      with TFileStream.Create(AOutFileName,fmCreate) do
      begin
        MStream.Position :=0;
        CopyFrom(MStream,MStream.Size);
        Free;
      end;
      MStream.Free;
      Result := True;
...
begin
  ...
  while i <= nLen do
  begin
    if (AUrl[i] in ['A'..'Z', 'a'..'z', '0'..'9']) or (AUrl[i] = '.') then
      Result := Result + AUrl[i]
    else
    begin
      s := IntToHex( Integer(AUrl[i]), 0 );
      if Length(s) = 1 then
        Result := Result + '%0' + s
      else
        Result := Result + '%' + s;
...
begin
  ...
  begin
    if AUrl[i] = '%' then
    begin
      tmp := AUrl[i + 1] + AUrl[i + 2];
      Result[nIdx] := Char( StrToInt( '$' + tmp ) );
      Inc(i, 3);
    end
    else
    begin
      Result[nIdx] := Char( AUrl[i] );
      Inc(i);
    end;
    Inc(nIdx);
  end;
  SetLength( Result, nIdx - 1);
end;
