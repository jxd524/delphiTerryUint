...
function xdMD5Buffer(const Buffer: PByte; const Size: Integer): TMD5Digest;
function MD5String(M: string): TMD5Digest;
function MD5File(N: string): TMD5Digest; overload;
function MD5File(const AFileName: string; const AHashSize: Integer): TMD5Digest; overload;
function MD5Print(D: TMD5Digest): string; overload;
function MD5Print(D: string): string; overload;
...
procedure MD5Final(var Context: MD5Context; var Digest: TMD5Digest);
var
	Bits: MD5CBits;
	Index: longword;
	PadLen: longword;
...
function MD5File(const AFileName: string; const AHashSize: Integer): TMD5Digest;
var
	FileHandle: THandle;
	MapHandle: THandle;
	ViewPointer: pointer;
...
begin
	...
	if FileHandle <> INVALID_HANDLE_VALUE then try
        ...
        if (AHashSize < nFileSize) and (AHashSize > 0) then
          nFileSize := AHashSize;
				MD5Update(Context, ViewPointer, nFileSize);
			finally
				UnmapViewOfFile(ViewPointer);
			end;
		finally
...
begin
  MD5Init(context);
  MD5Update(context, PChar(Buffer), Size);
  MD5Final(context, Result);
end;
