...
type
  TOverlapStyle = (osAccept, osRecv, osSend);
  ...
  private
    FSocket: TSocket;
    FLocalAddress: TSockAddr;      //FLocalAddress.sin_port = 0
    FRemoteAddress: TSockAddr;     //FRemoteAddress = 0           时；用户连接之后就断开了
    FPostRecvCount: Integer;       //=0时，自动断开SOCKET，回收相应内存块
    FPostSendCount: Integer;       //=0时，主动关闭不会丢失数据，不然可能会有数据丢失
    FLock: TRTLCriticalSection;
  public
    function IsCloseByServer: Boolean;
    function IsOnlyConnect: Boolean;
...
begin
  if Ap^.FPostSendCount <> 0 then
    OutputDebugString( '可能会选择发送的数据' );
  CloseSocketEx( Ap^.FSocket );
  PostNotifyIO;
end;
...
begin
  try
    ...
    begin
      ...
      begin
        //某种情况下，需要调用 FPendingManage.CheckPending;
        Continue;
      end;
...
procedure TxdCompletionPortServer.PostNotifyIO;
begin
...
begin
  ...
  begin
    ...
    begin
      OnError( '已经达到最大在线用户量，无法继续投递接收客户端: PostAcceptEx' );
      Break;
    end;
    ...
    begin
      ...
      begin
        ...
        //无法检查，但并不影响使用，如果不是恶意连接（只连接不发数据）。不会出问题，否则服务器资源可能被恶意消光
      end;
    end
    else
    begin
...
begin
  ...
  InterlockedIncrement( pSocket^.FPostSendCount );
  Result := __SendBufferEx( pSocket^.FSocket, @p^.FOverlap, p^.FBuffer, p^.FBufferLen );
  if not Result then
  begin
    InterlockedDecrement( pSocket^.FPostSendCount );
...
begin
  ...
  try
    OnDisConnection( Ap );
    if Ap^.FSocket <> INVALID_SOCKET then
      __DisConnectEx( Ap^.FSocket, nil )
    else
...
begin
  try
    ...
    FActive := False;
    for i := 0 to IocpWorkThreadCount - 1 do
      PostNotifyIO;
    //释放完成端口
...
var
  nPostRectCount: Integer;
begin
  InterlockedDecrement( Ap^.FClientSocketInfo^.FPostSendCount );
  nPostRectCount := 0;
...
begin
  LockManage( True );
  try
    Ap^.FLocalAddress.sin_port := 0;
    Ap^.FRemoteAddress.sin_port := 0;
    FManage.FreeMem( Ap );
  finally
    LockManage( False );
  end;
...
begin
  ...
  with FManage do
  begin
    MaxHashNodeCount := FOwner.MaxClientCount;
    HashTableCount := FOwner.MaxClientCount;
    UniquelyID := True;
    Active := True;
  end;
end;
...
function TClientSocketInfo.IsOnlyConnect: Boolean;
begin
  Result := (FLocalAddress.sin_port = 0)
end;
