...
type
  ...
  TStaticBufferManage = class
  public
    function GetMem(var P: Pointer): Boolean;
    function FreeMem(P: Pointer): Boolean;

    function InsertNode(const Value: Pointer): Boolean;
    function GetNode(var Value: Pointer): Boolean;
    
    constructor Create(const ABlockSize: Integer; ACapacity: Integer = MaxQueueLength);
    destructor  Destroy; override;
  private
    FMem: TStaticMemoryManager;
    FFreeQueue: TPointerQueue;
...
constructor TStaticBufferManage.Create(const ABlockSize: Integer; ACapacity: Integer);
begin
  FMem := TStaticMemoryManager.Create( ABlockSize, ACapacity );
  FFreeQueue := TPointerQueue.Create( ACapacity );
end;

destructor TStaticBufferManage.Destroy;
begin
...
function TStaticBufferManage.FreeMem(P: Pointer): Boolean;
begin
  Result := FMem.FreeMem(p);
end;

function TStaticBufferManage.GetMem(var P: Pointer): Boolean;
begin
  Result := FMem.GetMem(p);
end;

function TStaticBufferManage.GetNode(var Value: Pointer): Boolean;
begin
  Result := FFreeQueue.GetFirstNode( Value );
end;

function TStaticBufferManage.GetNodeCount: Integer;
begin
  Result := FFreeQueue.Count;
end;

function TStaticBufferManage.InsertNode(const Value: Pointer): Boolean;
begin
  Result := FFreeQueue.InsertNode( Value );
end;
