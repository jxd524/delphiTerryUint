...
类说明  :
          ...
          4: TxdUdpIoHandle: UDP包处理服务中心，对数据进行协议封装。提供大数据的发送与接收重组；
             工作模式：接收数据（N个线程）--> 放入接收缓存管理器中(1个) --> 处理缓存(m个)
...
外网环境下,UDP包大小为548,速度理想,丢包情况理想  CtMTU = 548 + 20 + 8  丢包率大概为：千分之0.2594
...
{本身使用常规常量}
  ...
  CtCombiPackageSize = CtMaxUdpSize - CtCombiPackageHeadSize;   //每个组合包最大数据区长度
  CtMaxCombiPackageSize = CtMaxCombiPackageCount * CtCombiPackageSize; //最多可发送数据长度
...
type
  ...
  PRecvBufferInfo = ^TRecvBufferInfo;
  TRecvBufferInfo = record
    FIP: Cardinal;
    FPort: Word;
    FLen: Word;
    FRecvPackage: array[0..CtMaxUdpSize - 1] of Byte;
  end;
  ...
  protected
    ...
    procedure DoThreadHandleRecvBuffer;
    procedure DoThreadHandleBuffer(const AIP: Cardinal; const APort: Word; const ApBuffer: pAnsiChar; const ABufferSize: Integer);
    ...
    {让子类处理的虚函数}
       //成功发送数据之后
    procedure DoSuccessSendBuffer(const ASendToIP: Cardinal; const ASendToPort: Word; const ApBuffer: PAnsiChar; const ABufLen: Integer); virtual;
       //当接收到组合包时，由提供的参数，得到一个有效的ID，以便TRecvCombiBufferManage进行处理
  ...
  private
  {对象定义}
    FRecvEvent: Cardinal;                       //唤醒处理缓存事件
    FRecvBufferManage: TxdFixedMemoryManager;   //接收缓存管理器
    FRecvBufferList: TPointerQueue;             //接收缓存列表
  ...
  private
    ...
    FRecvMaxBufferCount: Integer;
    FHandleBufferThreadCount: Integer;
    FDropPackageCount: Integer;
    FCurHandleBufferThreadCount: Integer;
    ...
    procedure SetRecvMaxBufferCount(const Value: Integer);
    procedure SetHandleBufferThreadCount(const Value: Integer);
    function  GetRecvBufferCount: Integer;
    function  GetRecvWaitToHandlePackageCount: Integer;
  published
  {接收缓存设置}
    property RecvMaxBufferCount: Integer read FRecvMaxBufferCount write SetRecvMaxBufferCount; //最大接收缓存数量,数量过小，会让类丢包(短时间内需要缓存的数据)
    property HandleBufferThreadCount: Integer read FHandleBufferThreadCount write SetHandleBufferThreadCount; //处理缓存线程数量
    property CurHandleBufferThreadCount: Integer read FCurHandleBufferThreadCount; //当前运行中的接收线程数
    property RecvBufferCount: Integer read GetRecvBufferCount; //当前接收缓存使用数量
    property RecvWaitToHandlePackageCount: Integer read GetRecvWaitToHandlePackageCount; //当前等待处理的缓存包数量
  {组合包缓存内存属性}
  ...
  {统计信息}
    ...
    property DropPackageCount: Integer read FDropPackageCount;                       //因接收缓存不足，而自动丢弃的包数量
...
const
  ...
  CtRecvBufferInfoSize = SizeOf(TRecvBufferInfo);
...
begin
  inherited;
  {接收缓存设置}
  FRecvMaxBufferCount := 100000;
  FHandleBufferThreadCount := 3;
  {发送组合包相关设置}
...
begin
  ...
  FreeAndNil( FRecvBufferList );
  FreeAndNil( FRecvBufferManage );
...
begin
  ...
  try
    InitStatValue;
    {接收缓存}
    FRecvBufferManage := TxdFixedMemoryManager.Create( CtRecvBufferInfoSize, FRecvMaxBufferCount );
    FRecvBufferList := TPointerQueue.Create( FRecvMaxBufferCount );
    FRecvEvent := CreateEvent( nil, True, False, nil );
    for i := 0 to FHandleBufferThreadCount - 1 do
      RunningByThread( DoThreadHandleRecvBuffer );
...
var
  ...
  nLen: Integer;
  addr: TSockAddrIn;
  pBuf: PRecvBufferInfo;
begin
  nLen := CtMaxUdpSize;
  if not __RecvBuffer( Package, nLen, addr ) then
  begin
    //当错误为： 远程主机强迫关闭了一个现有的连接。 (10054)，不会影响UDP通讯，
    //错误说明：IP与PORT指向的主机存在，并且数据到达了主机的路由，但是被路同主动丢弃了，否则不会提示此错误
  ...
  if nLen <= 0 then
  begin
    DoErrorInfo( '接收到数据长度小于0，自动丢弃' );
    Exit;
  end;

  if FRecvBufferManage.GetMem(pointer(pBuf)) then
  begin
    pBuf^.FIP := addr.sin_addr.S_addr;
    pBuf^.FPort := htons(addr.sin_port);
    pBuf^.FLen := nLen;
    Move( Package[0], pBuf^.FRecvPackage[0], nLen );
    if FRecvBufferList.Put(pBuf) then
    begin
      SetEvent( FRecvEvent );
    end
    else
    begin
      DoErrorInfo('接收到的数据无法添加到缓存队列中');
      InterlockedIncrement(FDropPackageCount);
      FRecvBufferManage.FreeMem(pBuf)
    end;
  end
  else
  begin
    DoErrorInfo( '接收缓存不足，将自动丢弃此包' );
    InterlockedIncrement(FDropPackageCount);
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      pCombi^.FLastActiveTime := GetTickCount;
    finally
...
procedure TxdUdpIoHandle.DoSuccessSendBuffer(const ASendToIP: Cardinal; const ASendToPort: Word; const ApBuffer: PAnsiChar; const ABufLen: Integer);
begin

end;

procedure TxdUdpIoHandle.DoThreadHandleBuffer(const AIP: Cardinal; const APort: Word; const ApBuffer: pAnsiChar; const ABufferSize: Integer);
var
  ...
  nLen: Integer;
begin
  if ABufferSize <= CtSinglePackageHeadSize then
  begin
    InterlockedIncrement( FRecvErrorPackageCount );
    DoErrorInfo( '接收到的数据包长度过小: %d', [ABufferSize] );
    Exit;
  end;
  //单包   版本(1) - 此包总长度(2) - CRC32码(4) - 包信息(1)[前四位：0； 后四位: 协议命令，当为0时，表示需要由子类处理]
  //组合包 版本(1) - 此包总长度(2) - CRC32码(4) - 包信息(1)[前四位：个数； 后四位: 包顺序] - 标志(2)
  nVer := Byte(ApBuffer[0]);
  ...
  Move( ApBuffer[1], nPackageLen, 2 );
  if nPackageLen <> ABufferSize then
  begin
    InterlockedIncrement( FRecvErrorPackageCount );
    DoErrorInfo( '包的长度有误' );
    Exit;
  end;
  Move( ApBuffer[3], nCrc32Code, 4 );
  Move( ApBuffer[7], nPackageInfo, 1);
  ParsePackageInfo( nPackageInfo, nPackageCount, nPackageIndex );
  if nPackageCount = 0 then
  begin
    //独立包
    pData := @ApBuffer[CtSinglePackageHeadSize];
    nLen := ABufferSize - CtSinglePackageHeadSize;
    ...
    nCmd := nPackageIndex;
    if nCmd = 0 then
      OnRecvBuffer( AIP, APort, pData, nLen )
    else
    begin
      case nCmd of
        CtReGetCombiInfoCmd: DoRecvReGetCombiPackage( AIP, APort, pData, nLen );
        else
        begin
          InterlockedIncrement( FRecvErrorPackageCount );
  ...
  begin
    //组合包
    if ABufferSize <= CtCombiPackageHeadSize then
    begin
      InterlockedIncrement( FRecvErrorPackageCount );
      DoErrorInfo( '接收到组合包长度过小： %d', [ABufferSize] );
      Exit;
    end;
    Move( ApBuffer[8], nCombiID, 2 );
    pData := @ApBuffer[CtCombiPackageHeadSize];
    nLen := ABufferSize - CtCombiPackageHeadSize;
    if not DecodeBuffer( nCrc32Code, pData, nLen ) then
    begin
      InterlockedIncrement( FRecvErrorPackageCount );
      DoErrorInfo( 'CRC32解密出错' );
    ...
    OnRecvCombiPackage( AIP, APort, pData, nLen, nPackageCount, nPackageIndex, nCombiID );
  end;
end;

procedure TxdUdpIoHandle.DoThreadHandleRecvBuffer;
var
  pBuf: PRecvBufferInfo;
begin
  InterlockedIncrement( FCurHandleBufferThreadCount );

  try
    WaitForSingleObject( FRecvEvent, INFINITE );
    while Active do
    begin
      ResetEvent( FRecvEvent );
  //    OutputDebugString( PChar('Thread Active : ' + IntToStr(GetCurrentThreadId)) );
      while FRecvBufferList.Get(Pointer(pBuf)) do
      begin
        try
  //        OutputDebugString( PChar('Thread Handle Buffer : ' + IntToStr(GetCurrentThreadId)) );
          try
            DoThreadHandleBuffer( pBuf^.FIP, pBuf^.FPort, @pBuf^.FRecvPackage, pBuf^.FLen );
          except
            OutputDebugString( 'eeeeeeeerrrrrrrrr' );
          end;
        finally
          FRecvBufferManage.FreeMem( pBuf );
        end;
      end;
  //    OutputDebugString( PChar('Thread Sleep: ' + IntToStr(GetCurrentThreadId)) );
      WaitForSingleObject( FRecvEvent, INFINITE );
    end;
  finally
    InterlockedDecrement( FCurHandleBufferThreadCount );
  end;
end;
...
function TxdUdpIoHandle.GetRecvBufferCount: Integer;
begin
  if Active then
    Result := FRecvBufferManage.Capacity - FRecvBufferManage.Count
  else
    Result := 0;
end;
...
function TxdUdpIoHandle.GetRecvWaitToHandlePackageCount: Integer;
begin
  if Active then
    Result := FRecvBufferList.Count
  else
    Result := 0;
end;
...
begin
  ...
  else if ABufLen <= CtMaxCombiPackageSize then
    Result := _SendCombiPackage( AIP, APort, ApBuffer, ABufLen )
  else
  begin
    Result := -1;
    DoErrorInfo( '超过最大允许发送长度, 请自行分割数据' );
  end;
  if Result = ABufLen then
  begin
    InterlockedIncrement( FSendFullPackageCount );
    DoSuccessSendBuffer( AIP, APort, ApBuffer, ABufLen );
  end;
end;
...
procedure TxdUdpIoHandle.SetHandleBufferThreadCount(const Value: Integer);
begin
  if not Active and (Value > 0) then
    FHandleBufferThreadCount := Value;
end;

procedure TxdUdpIoHandle.SetRecvMaxBufferCount(const Value: Integer);
begin
  if not Active and (Value > 0) then
    FRecvMaxBufferCount := Value;
end;
